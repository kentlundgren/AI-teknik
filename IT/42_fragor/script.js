...
function showResult(){ quiz.classList.add('hidden'); result.classList.remove('hidden'); const score=questions.reduce((sum,q,i)=>sum+(same(answers[i]||[],q.correct)?q.points:0),0); const total=questions.reduce((sum,q)=>sum+q.points,0); const pct=Math.round(score/total*100); const name=participant ? ` ${escapeHtml(participant)}` : ''; const review=questions.map((q,i)=>{const ok=same(answers[i]||[],q.correct); const answerText=(answers[i]||[]).map(n=>q.options[n]).join(', ')||'Inget svar'; const correctText=q.correct.map(n=>q.options[n]).join(', '); return `<article class="result-review ${ok?'correct-answer':'wrong-answer'}"><h3>${i+3}. ${ok?'Rätt':'Fel'} <span aria-hidden="true">${ok?'✓':'×'}</span></h3><p><strong>Ditt svar:</strong> ${escapeHtml(answerText)}<br><strong>Rätt svar:</strong> ${escapeHtml(correctText)}<br>${q.explanation}</p></article>`;}).join('');
